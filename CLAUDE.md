...
> - ✅ ★ **検出器は 2026-08-21 に初めて実モデルへ当たった**(ラン `detector-firstlight-01`)。
>   `drop` / `p_value` / `adjusted_lcb` が**実モデルに対して初めて計算された**
>
...
>
> ★ **ラン `detector-firstlight-01` は 2026-08-21 に完走した(≒ $26.5・GPU は terminate 済み)。**
> **関門 G0〜G5 全通過 / a は k=3 で頑健に合格。判定 B 合格**(`drop` +4.47pt・`p_holm` 8.61e-11)。
> ⛔ **判定 A は不合格** —— **素のベースも `detected = true`**(`drop` +1.41pt・`p_holm` 0.0146)。
> ★ `n`=4,742 では臨界値が ≒1.25pt しかなく、**`adjusted_lcb > 0` は汚染の印として働かない。**
> ⛔ **これを「検出器の偽陽性」と断定しない**(摂動の難しさと、ベース自身の汚染を分離できていない)。
> ✅ **副次の読みは 2026-08-22 に回収**(追加課金ゼロ)—— 汚染アームの `drop` は
> **注入した問題に集中**(注入済み +9.92pt / 非注入 +0.84pt・有意ですらない)。⛔ **報告のみ。**
> ★ **次の着手は、陰性対照 `x00` アーム(0% 注入・同一レシピ・同一 `T`)の事前登録から。**
> ⛔ **課金と規則の選択を伴うので、ユーザーの判断を仰いでから凍結する。**
