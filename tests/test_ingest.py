...
class Test無害な選択肢を残す:
    @pytest.mark.parametrize(
        ...
        [
            ...
            "これらの惑星を構成する物質は、主にガスではなくプラズマの形をしている。",
            # ごく普通の選択肢。
            "0,4",
            "あなたからのその天体の距離",
        ],
...
class Test実データ:
    ...
    def test_実データの1行目は通る(self) -> None:
        line = (
            "ある物体の実際の明るさと、あなたがいる位置からの見かけの明るさの両方が"
            "わかっている場合、他の情報がなくても推定することができることは次のどれか？, "
            "あなたに対するその天体の速度 , その天体の組成 , その天体の大きさ , "
            "あなたからのその天体の距離,D"
        )
        parsed = parse_row(next(csv.reader([line])))

        assert not isinstance(parsed, str)
        assert parsed[1] == "あなたからのその天体の距離"
