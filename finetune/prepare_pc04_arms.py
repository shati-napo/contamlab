#!/usr/bin/env python3
"""finetune/prepare_pc04_arms.py — ラン positive-control-04 の注入集合を用意する。

    python finetune/prepare_pc04_arms.py

preregister「ラン: positive-control-04」→「実装」の実装。
...
★ なぜ複製が要るのか —— 器(`65-manipulation-check.sh` / `train_lora.py`)は
  **アーム名からファイルを引く**(`data/injection/{arm}.jsonl`)。段ごとに別のモデル名が
  要る(応答キャッシュのキーがモデル名だから)ので、段の数だけ同名のファイルが要る。

★ pc-02 の `pcr*-x40` と名前を分ける理由 —— pc-04 は**同じ梯子を別のベースで**回す。
  同じアーム名を使い回すと、**pc-02 の応答キャッシュと混ざる**(キーはモデル名)。
  pc-02 は1本も学習していないのでキャッシュは空だが、**名前で守るほうが確実である。**
...
def pc04_arms() -> dict[str, str]:
    """段 → 複製元。名前は train_lora.py の凍結表から引く(手で打ち直さない)。"""
    from train_lora import recipe_arms
    return {arm: "pc-x40" for arm in recipe_arms("positive-control-04").values()}
...
def main() -> int:
    ...
    for src in sorted(set(arms.values())):
        ...
        if got[".jsonl"] != want["txt_sha256"] or got[".ids"] != want["ids_sha256"]:
            ...
            return 1
        # ★ pc-03 は「注入 0 問であること」を確かめた。pc-04 は逆で、
        #   **1,896 問すべて入っていること**を確かめる。取り違えると測るものが変わる。
    ...
    (d / "manifest-pc04.json").write_text(json.dumps({
        "run": "positive-control-04",
        "note": "pc-01 の注入集合をアーム名だけ変えて複製したもの。中身は生成していない。"
                "pc-02 の pcr*-x40 とは別名にしてある(応答キャッシュのキーがモデル名のため)。",
    ...
    print(f"記録: {d / 'manifest-pc04.json'}")
    return 0
