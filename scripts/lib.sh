...
# ★ ベンチマーク本体の**生の** sha256(先頭8桁)。2026-08-09 に pc-03 で追加した。
#
#   pc-02 で判明した —— **manifest が一致してもバイトは一致しないことがある。**
#   `$BENCHMARK` は .gitignore 済みで各環境で作り直すため、作った OS によって
#   問題文中の改行が `\r\n`(Windows)と `\n`(Linux)に割れる。件数も id も一致するので
#   **上の JMMLU_EXPECTED_* による manifest 照合はこの差を検出しない。**
#   ラン pc-01 / pc-02 が実際に測ったのは Linux で作った版である。
#
#   ★ 2026-08-09、ラン pc-03 の実行で**全 64 桁が記録に残った**(先頭 8 桁だけだったのを
#     ここで埋めた)。pc-03 の実行環境節および reports/positive-control-03/ が出典。
#     **これは測定値の記録であって、判定規則ではない。**
JMMLU_MEASURED_SHA256_PREFIX="8aa877e57335daca61a9aa4e676e78e1da5a7608806f6e959e1e67bc56317745"
