...
### 3. 実行環境 —— **02 は記録済み / 03 は借りるまで埋まらない**

`30-record-environment.sh` が生成する `reports/environment.<tag>.md` を
preregister の**そのランの**「実行環境」枠に**そのまま貼る。**
Ollama の版・GGUF の SHA256・GPU・`OLLAMA_NUM_PARALLEL=1` などが入っている。

| ラン | 状態 |
|---|---|
| `jmmlu-shuffle-02` | ✅ **記録済み**(`lambda-a100-40gb-20260806`・2026-08-06) |
| `jmmlu-shuffle-03` | ⬜ **枠だけ用意(2026-08-07)。** ホストを借りた時点で埋まる |

⚠️ **02 の記録を 03 に流用しない。** そのホストは terminate 済みで、03 は別のホストを借りる。
GPU・ドライバ・Ollama の版・GGUF の SHA256 はどれも変わりうる。
**変わっていないことを確認する手段は再記録以外に無い。** 応答キャッシュも新しいタグで切る。

**`30` の直後・`35-select-format.sh` の前に貼る。** 後から書くと「実際に使った環境の記録」
...
### 写像表は**モデル別 ψ の最大**で引く(2026-08-08 から)

`60-production.sh` はパイロット②の 2×2 表からモデルごとに ψ̂ を出し、
**その Clopper-Pearson 上側が最大のモデル**で写像表を引く。**プール(全モデル平均)では引かない。**
採用基準はモデルごとに照合されるので、基準3「事前の検出力 ≥ 0.80」も
モデルごとに要求される —— 最も不利なモデルで設計するのが唯一の整合解である。
根拠は [preregister.md](../preregister.md) の
「★ M ≥ 2 のとき、どの ψ で写像表を引くか」。

> [!warning] ⚠️ **モデル別の検出力は harness の出力に無い。**
> [harness.py:175](../contamlab/harness.py#L175) は全モデルを平均した不一致率で
> `observed_power` を計算する(編集禁止領域なので直せない)。
> **採用基準3 の照合には `60-production.sh` が印字するモデル別の値を使い、報告に載せる。**

**M ≠ 2 なら停止する。** 写像表は M=2 / 実効 α=0.0250 で作られており、
ロースターの本数が変われば表ごと引き直しになる。**流用しない。**
